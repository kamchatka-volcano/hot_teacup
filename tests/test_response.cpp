...
TEST(Response, Status)
{
    ...
    EXPECT_EQ(response.toString(), "HTTP/1.1 100 Continue\r\n\r\n");
    response = http::Response{Status::_101_Switching_Protocol};
    EXPECT_EQ(response.toString(), "HTTP/1.1 101 Switching Protocol\r\n\r\n");
    response = http::Response{Status::_102_Processing};
    EXPECT_EQ(response.toString(), "HTTP/1.1 102 Processing\r\n\r\n");
    response = http::Response{Status::_103_Early_Hints};
    EXPECT_EQ(response.toString(), "HTTP/1.1 103 Early Hints\r\n\r\n");
    ...
    EXPECT_EQ(response.toString(), "HTTP/1.1 201 Created\r\n\r\n");
    response = http::Response{Status::_202_Accepted};
    EXPECT_EQ(response.toString(), "HTTP/1.1 202 Accepted\r\n\r\n");
    response = http::Response{Status::_203_Non_Authoritative_Information};
    EXPECT_EQ(response.toString(), "HTTP/1.1 203 Non-Authoritative Information\r\n\r\n");
    ...
    EXPECT_EQ(response.toString(), "HTTP/1.1 205 Reset Content\r\n\r\n");
    response = http::Response{Status::_206_Partial_Content};
    EXPECT_EQ(response.toString(), "HTTP/1.1 206 Partial Content\r\n\r\n");
    response = http::Response{Status::_207_Multi_Status};
    EXPECT_EQ(response.toString(), "HTTP/1.1 207 Multi-Status\r\n\r\n");
    response = http::Response{Status::_208_Already_Reported};
    EXPECT_EQ(response.toString(), "HTTP/1.1 208 Already Reported\r\n\r\n");
    response = http::Response{Status::_226_IM_Used};
    EXPECT_EQ(response.toString(), "HTTP/1.1 226 IM Used\r\n\r\n");
    response = http::Response{Status::_300_Multiple_Choice};
    EXPECT_EQ(response.toString(), "HTTP/1.1 300 Multiple Choice\r\n\r\n");
    response = http::Response{Status::_301_Moved_Permanently};
    EXPECT_EQ(response.toString(), "HTTP/1.1 301 Moved Permanently\r\n\r\n");
    response = http::Response{Status::_302_Found};
    EXPECT_EQ(response.toString(), "HTTP/1.1 302 Found\r\n\r\n");
    response = http::Response{Status::_303_See_Other};
    EXPECT_EQ(response.toString(), "HTTP/1.1 303 See Other\r\n\r\n");
    response = http::Response{Status::_304_Not_Modified};
    EXPECT_EQ(response.toString(), "HTTP/1.1 304 Not Modified\r\n\r\n");
    response = http::Response{Status::_307_Temporary_Redirect};
    EXPECT_EQ(response.toString(), "HTTP/1.1 307 Temporary Redirect\r\n\r\n");
    response = http::Response{Status::_308_Permanent_Redirect};
    EXPECT_EQ(response.toString(), "HTTP/1.1 308 Permanent Redirect\r\n\r\n");
    response = http::Response{Status::_400_Bad_Request};
    EXPECT_EQ(response.toString(), "HTTP/1.1 400 Bad Request\r\n\r\n");
    response = http::Response{Status::_401_Unauthorized};
    EXPECT_EQ(response.toString(), "HTTP/1.1 401 Unauthorized\r\n\r\n");
    response = http::Response{Status::_402_Payment_Required};
    EXPECT_EQ(response.toString(), "HTTP/1.1 402 Payment Required\r\n\r\n");
    response = http::Response{Status::_403_Forbidden};
    EXPECT_EQ(response.toString(), "HTTP/1.1 403 Forbidden\r\n\r\n");
    ...
    EXPECT_EQ(response.toString(), "HTTP/1.1 405 Method Not Allowed\r\n\r\n");
    response = http::Response{Status::_406_Not_Acceptable};
    EXPECT_EQ(response.toString(), "HTTP/1.1 406 Not Acceptable\r\n\r\n");
    response = http::Response{Status::_407_Proxy_Authentication_Required};
    EXPECT_EQ(response.toString(), "HTTP/1.1 407 Proxy Authentication Required\r\n\r\n");
    response = http::Response{Status::_408_Request_Timeout};
    EXPECT_EQ(response.toString(), "HTTP/1.1 408 Request Timeout\r\n\r\n");
    response = http::Response{Status::_409_Conflict};
    EXPECT_EQ(response.toString(), "HTTP/1.1 409 Conflict\r\n\r\n");
    response = http::Response{Status::_410_Gone};
    EXPECT_EQ(response.toString(), "HTTP/1.1 410 Gone\r\n\r\n");
    response = http::Response{Status::_411_Length_Required};
    EXPECT_EQ(response.toString(), "HTTP/1.1 411 Length Required\r\n\r\n");
    response = http::Response{Status::_412_Precondition_Failed};
    EXPECT_EQ(response.toString(), "HTTP/1.1 412 Precondition Failed\r\n\r\n");
    response = http::Response{Status::_413_Payload_Too_Large};
    EXPECT_EQ(response.toString(), "HTTP/1.1 413 Payload Too Large\r\n\r\n");
    response = http::Response{Status::_414_URI_Too_Long};
    EXPECT_EQ(response.toString(), "HTTP/1.1 414 URI Too Long\r\n\r\n");
    response = http::Response{Status::_415_Unsupported_Media_Type};
    EXPECT_EQ(response.toString(), "HTTP/1.1 415 Unsupported Media Type\r\n\r\n");
    response = http::Response{Status::_416_Range_Not_Satisfiable};
    EXPECT_EQ(response.toString(), "HTTP/1.1 416 Range Not Satisfiable\r\n\r\n");
    response = http::Response{Status::_417_Expectation_Failed};
    EXPECT_EQ(response.toString(), "HTTP/1.1 417 Expectation Failed\r\n\r\n");
    response = http::Response{Status::_418_Im_a_teapot};
    EXPECT_EQ(response.toString(), "HTTP/1.1 418 I'm a teapot\r\n\r\n");
    response = http::Response{Status::_421_Misdirected_Request};
    EXPECT_EQ(response.toString(), "HTTP/1.1 421 Misdirected Request\r\n\r\n");
    response = http::Response{Status::_422_Unprocessable_Entity};
    EXPECT_EQ(response.toString(), "HTTP/1.1 422 Unprocessable Entity\r\n\r\n");
    response = http::Response{Status::_423_Locked};
    EXPECT_EQ(response.toString(), "HTTP/1.1 423 Locked\r\n\r\n");
    response = http::Response{Status::_424_Failed_Dependency};
    EXPECT_EQ(response.toString(), "HTTP/1.1 424 Failed Dependency\r\n\r\n");
    response = http::Response{Status::_425_Too_Early};
    EXPECT_EQ(response.toString(), "HTTP/1.1 425 Too Early\r\n\r\n");
    response = http::Response{Status::_426_Upgrade_Required};
    EXPECT_EQ(response.toString(), "HTTP/1.1 426 Upgrade Required\r\n\r\n");
    response = http::Response{Status::_428_Precondition_Required};
    EXPECT_EQ(response.toString(), "HTTP/1.1 428 Precondition Required\r\n\r\n");
    response = http::Response{Status::_429_Too_Many_Requests};
    EXPECT_EQ(response.toString(), "HTTP/1.1 429 Too Many Requests\r\n\r\n");
    response = http::Response{Status::_431_Request_Header_Fields_Too_Large};
    EXPECT_EQ(response.toString(), "HTTP/1.1 431 Request Header Fields Too Large\r\n\r\n");
    response = http::Response{Status::_451_Unavailable_For_Legal_Reasons};
    EXPECT_EQ(response.toString(), "HTTP/1.1 451 Unavailable For Legal Reasons\r\n\r\n");
    response = http::Response{Status::_500_Internal_Server_Error};
    EXPECT_EQ(response.toString(), "HTTP/1.1 500 Internal Server Error\r\n\r\n");
    response = http::Response{Status::_501_Not_Implemented};
    EXPECT_EQ(response.toString(), "HTTP/1.1 501 Not Implemented\r\n\r\n");
    response = http::Response{Status::_502_Bad_Gateway};
    EXPECT_EQ(response.toString(), "HTTP/1.1 502 Bad Gateway\r\n\r\n");
    response = http::Response{Status::_503_Service_Unavailable};
    EXPECT_EQ(response.toString(), "HTTP/1.1 503 Service Unavailable\r\n\r\n");
    response = http::Response{Status::_504_Gateway_Timeout};
    EXPECT_EQ(response.toString(), "HTTP/1.1 504 Gateway Timeout\r\n\r\n");
    response = http::Response{Status::_505_HTTP_Version_Not_Supported};
    EXPECT_EQ(response.toString(), "HTTP/1.1 505 HTTP Version Not Supported\r\n\r\n");
    response = http::Response{Status::_506_Variant_Also_Negotiates};
    EXPECT_EQ(response.toString(), "HTTP/1.1 506 Variant Also Negotiates\r\n\r\n");
    response = http::Response{Status::_507_Insufficient_Storage};
    EXPECT_EQ(response.toString(), "HTTP/1.1 507 Insufficient Storage\r\n\r\n");
    response = http::Response{Status::_508_Loop_Detected};
    EXPECT_EQ(response.toString(), "HTTP/1.1 508 Loop Detected\r\n\r\n");
    response = http::Response{Status::_510_Not_Extended};
    EXPECT_EQ(response.toString(), "HTTP/1.1 510 Not Extended\r\n\r\n");
    response = http::Response{Status::_511_Network_Authentication_Required};
    EXPECT_EQ(response.toString(), "HTTP/1.1 511 Network Authentication Required\r\n\r\n");
    ...
    response = http::Response{Status::_404_Not_Found, "Not Found", http::ContentTypeString{"text/csv"}};
...
TEST(Response, StatusCGI)
{
    ...
    EXPECT_EQ(response.toString(http::ResponseMode::Cgi), "Status: 200 OK\r\n\r\n");
    response = http::Response{Status::_404_Not_Found};
    EXPECT_EQ(response.toString(http::ResponseMode::Cgi), "Status: 404 Not Found\r\n\r\n");
    response = http::Response{Status::_404_Not_Found, "Not Found"};
    EXPECT_EQ(
            response.toString(http::ResponseMode::Cgi),
...
TEST(Response, Redirect)
{
    ...
    EXPECT_EQ(response.toString(), "HTTP/1.1 302 Found\r\nLocation: /\r\n\r\n");
    response = http::Response{http::Redirect{"/", http::RedirectType::SeeOther}};
    EXPECT_EQ(response.toString(), "HTTP/1.1 303 See Other\r\nLocation: /\r\n\r\n");
    response = http::Response{http::Redirect{"/", http::RedirectType::NotModified}};
    EXPECT_EQ(response.toString(), "HTTP/1.1 304 Not Modified\r\nLocation: /\r\n\r\n");
    response = http::Response{http::Redirect{"/", http::RedirectType::MultipleChoice}};
    EXPECT_EQ(response.toString(), "HTTP/1.1 300 Multiple Choice\r\nLocation: /\r\n\r\n");
    response = http::Response{http::Redirect{"/", http::RedirectType::MovedPermanently}};
    EXPECT_EQ(response.toString(), "HTTP/1.1 301 Moved Permanently\r\nLocation: /\r\n\r\n");
    response = http::Response{http::Redirect{"/", http::RedirectType::PermanentRedirect}};
    EXPECT_EQ(response.toString(), "HTTP/1.1 308 Permanent Redirect\r\nLocation: /\r\n\r\n");
    response = http::Response{http::Redirect{"/", http::RedirectType::TemporaryRedirect}};
    EXPECT_EQ(response.toString(), "HTTP/1.1 307 Temporary Redirect\r\nLocation: /\r\n\r\n");

    auto response2 = http::Response{http::Redirect{"/"}};
    EXPECT_EQ(response2.toString(), "HTTP/1.1 302 Found\r\nLocation: /\r\n\r\n");
    response2 = http::Redirect{"/", http::RedirectType::SeeOther};
    EXPECT_EQ(response2.toString(), "HTTP/1.1 303 See Other\r\nLocation: /\r\n\r\n");
}
...
TEST(ResponseView, ResponseFromResponseView)
{
    ...
    EXPECT_EQ(response.headers().at(0).value(), "/");
    EXPECT_EQ(response.headers().at(0).name(), "Location");
    EXPECT_EQ(response.header("Location"), "/");
    EXPECT_EQ(response.headers().at(0).value(), "/");
    EXPECT_EQ(response.body(), "Hello world");
}
